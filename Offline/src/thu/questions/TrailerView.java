...
public class TrailerView extends LinearLayout {
    ...
    public void Init(TrailerQuestion trailerQuestion, Helper helper) {
        ...
        for (int i = 0; i < trailerQuestion.ques.size(); ++i)
		{
            ...
            try {
                trail.text.setText(trailerQuestion.userWords.get(i));
                int btn = trailerQuestion.userAnswer.get(i);
                if (btn == 1) {
                    if (trail.ss == 1)
                        trail.easyScore.callOnClick();
                    else
                        trail.hardScore.callOnClick();
                } else if (btn == 2)
                    trail.palBtn.callOnClick();
                else if (btn == 3)
                    trail.relatedBtn.callOnClick();
                else if (btn == 4)
                    trail.notRelBtn.callOnClick();
            }
            catch (Exception e) {
